...
myEditor :: String
myEditor = myTerminal ++ "nano"    -- Sets nano as editor for tree select but I'm not going to use tree select.
...
myKeys =
    -- Xmonad
        ...
        , ("M-S-x", spawn "betterlockscreen --lock dimblur")
        , ("M-d", spawn "rofi -show drun -show-icons -combi-modi ")
    ...
    -- Emacs (CTRL-e followed by a key) I currently use nVim so don't need theese keybindings.
        ...
        --, ("C-e d", spawn "emacsclient -c -a '' --eval '(direvd nil)'")       -- dired emacs file manager
        --, ("C-e i", spawn "emacsclient -c -a '' --eval '(erc)'")             -- erc emacs irc client

    --- My Applications (Super+Shift+Key)
        , ("M-S-<F1>", spawn "brave-browser")
